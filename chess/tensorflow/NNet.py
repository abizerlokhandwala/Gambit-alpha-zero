...
class NNetWrapper(NeuralNet):
    ...
    def save_checkpoint(self, folder='/output', filename='checkpoint.pth.tar'):
        ...
        with self.nnet.graph.as_default():
            self.saver.save(self.sess, saveTo) #was initially filepath

    def load_checkpoint(self, folder='/output', filename='checkpoint.pth.tar'):
        # filepath = os.path.join(folder, filename)
        saveTo = folder+"/"+filename
        # if not os.path.exists(filepath+'.meta'):
        #     print("no model in PATH: ", filepath+'.meta')
        #     raise("No model in path {}".format(filepath))
        with self.nnet.graph.as_default():
            self.saver = tf.train.Saver()
            self.saver.restore(self.sess, saveTo) #was initially filepath
